...
const filtered =(name)=> b.filter(p=> p.name.includes(name));
function pronpt() {
...
function downloadPreset(pid, bankid) {
	...
	const dirname = `${pid}_${pheader[0].name.replace(" ","_")}`; //+"_"+pid}`
	...
	for (const z of zones) {
		...
		const fff = require("fs").createWriteStream(
			`${dirname}/${keyRange.lo}_${keyRange.hi}_${lo}_${hi}.wav`
		);
